...
nc=nc_open('2021/chl-Johnston-1998-2019-mean.nc')
v1=nc$var[[1]]
...
#breaks=h$breaks
breaks=log(c(seq(0.04,0.10,0.0001),0.73))
n=length(breaks)-1
...
axis(2)
axis(1,seq(190,192,0.5),seq(190,192,0.5)-360)
box()


par(mar=c(3,1,3,3))
image.scale(clim, col=c, breaks=log(c(seq(0.04,0.1,0.0001),0.11)), horiz=FALSE,xlab='',ylab='',main='',las=1,yaxt='n')
#image.scale(clim, col=c, breaks=breaks, horiz=FALSE,xlab='',ylab='',main='',las=1,yaxt='n')
axis(4,at=log(c(seq(0.04,0.1,0.01),0.11)),c(seq(0.04,0.1,0.01),0.73),las=1)
#axis(4,seq(breaks[1],breaks[length(breaks)],0.02),round(exp(seq(breaks[1],breaks[length(breaks)],0.02)),3),las=1)
box()

##################################################################################################################################3
#anomaly

nc=nc_open('2021/chl-Johnston-2020-mean.nc')
v1=nc$var[[1]]
...
breaks=c(-0.23,seq(-0.01,0.01,0.00005),0.07)
n=length(breaks)-1
...
image.scale(anom, col=c, breaks=c(-0.011,seq(-0.01,0.01,0.00005),0.011), horiz=FALSE,xlab='',ylab='',main='',yaxt='n',las=1)
axis(4,at=c(-0.012,seq(-0.01,0.01,0.002),0.012),labels=c(-0.23,seq(-0.01,0.01,0.002),0.07),las=1)
#axis(4,las=1)
box()
...
#time-series
nc=nc_open('2021/chl-Johnston-1998-2020.nc')
v1=nc$var[[1]]
chl=ncvar_get(nc,v1)
...
write.csv(T,'chl-Johnston.csv',row.names=TRUE)
