...
nc=nc_open('2021/chl-Mariana-1998-2019-mean.nc')
v1=nc$var[[1]]
...
#breaks=h$breaks
breaks=log(c(seq(0.033,0.07,0.00001),1.33))
n=length(breaks)-1
...
par(mar=c(3,1,3,4))
image.scale(clim, col=c, breaks=log(c(seq(0.033,0.07,0.00001),0.075)), horiz=FALSE,xlab='',ylab='',main='',las=1,yaxt='n')
axis(4,at=log(c(seq(0.035,0.07,0.005),0.075)),c(seq(0.033,0.07,0.005),1.33),las=1)
box()
##################################################################################################################################3
#anomaly

nc=nc_open('2021/chl-Mariana-2020-mean.nc')
v1=nc$var[[1]]
...
breaks=c(-0.18,seq(-0.02,0.02,0.00005),0.7)
n=length(breaks)-1
...
par(mar=c(3,1,3,4))
image.scale(anom, col=c, breaks=c(-0.025,seq(-0.02,0.02,0.00005),0.025), horiz=FALSE,xlab='',ylab='',main='',yaxt='n',las=1)
axis(4,at=c(-0.025,seq(-0.02,0.02,0.005),0.025),labels=c(-0.18,seq(-0.02,0.02,0.005),0.7),las=1)
box()


#time-series
nc=nc_open('2021/chl-Mariana-1998-2020.nc')
v1=nc$var[[1]]
chl=ncvar_get(nc,v1)
...
write.csv(T,'chl-Mariana.csv',row.names=TRUE)
