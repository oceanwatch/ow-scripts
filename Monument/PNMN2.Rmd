...
SST data used:  
NOAA Coral Reef Watch "CoralTemp", 1985-2018.  
...
n=dim(T1)[3]
ts=rep(NA,n)
for (i in 1:n) ts[i]=mean(T1[,,i],na.rm=TRUE)
...
tot=data.frame(as.character(dates),ts,anom)
names(tot)=c("date","mean_sst","sst_anomaly")
write.csv(tot,'sst-1985-2018-coraltemp-v1.dat',row.names=FALSE)
...
Data used: ESA CCI ocean color data, v.4.2, 1998 - 2019.  
...
tot=data.frame(as.character(dates[11:length(ts)]),ts2,anom)
names(tot)=c("date","mean_chl","chl_anomaly")
write.csv(tot,'chl-1998-2018-coraltemp-v1.dat',row.names=FALSE)
